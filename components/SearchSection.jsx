...
export default function SearchSection() {
  ...
  const handleSubmit = async (e) => {
    ...
    setIsLoading(true);
    router.push(`/genome/${username.trim()}`);
  };
      ...
      <div className="relative">
        <div className="relative flex items-center gap-2 p-2 bg-white rounded-2xl shadow-lg border border-[var(--color-border)] focus-within:ring-2 focus-within:ring-[var(--color-primary)] focus-within:ring-opacity-20 transition-all">
          ...
          <button
            ...
            className="px-6 py-3 bg-[var(--color-primary)] text-white font-medium rounded-xl hover:bg-[var(--color-primary-dark)] disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 flex items-center gap-2 shadow-sm hover:shadow-md"
          >
            {isLoading ? (
              <>
                <svg
        ...
        <div className="mt-4 flex flex-wrap items-center justify-center gap-2">
          ...
          {["torrenegra", "john", "maria"].map((suggestion) => (
            <button
              ...
              className="px-3 py-1.5 text-sm bg-white border border-[var(--color-border)] rounded-lg hover:border-[var(--color-primary)] hover:text-[var(--color-primary)] transition-colors"
            >
              {suggestion}
            </button>
          ))}
